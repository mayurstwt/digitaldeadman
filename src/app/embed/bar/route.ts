const script = String.raw`(() => {
  ...
  const status = (currentScript.dataset.ddmStatus || "pending").toLowerCase();

  if (status === "paid" || status === "inactive" || status === "off") {
    return;
  }
  ...
  const project = currentScript.dataset.ddmProject || "Website project";
  const client = currentScript.dataset.ddmClient || "client";
  const invoiceUrl = currentScript.dataset.ddmInvoiceUrl || "";
  const customMessage = currentScript.dataset.ddmMessage || "";
  const contactEmail = currentScript.dataset.ddmContactEmail || "";
